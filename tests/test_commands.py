...
def test_only_verified_rate_and_bandwidth_values_exist() -> None:
    ...
    `0x0A` 被有意排除，理由有两条：**本型号协议文档的速率表根本没有这个编码**
    （只有 `0x09:100Hz`、`0x0B:200Hz`；「125 Hz」出自维特跨型号的通用编码表，
    RAY-308 查明），而实测又证实了一遍——99.29 Hz，与 `0x09` 几乎相同，且同一次
    探测里 `0x0B` 跑到 198 Hz，所以不是链路带宽不足。

    **带宽七档的情形不同，别把这条测试读成「带宽也逐档实测过」**：七个标称值载于
    本型号官方协议文档（RAY-308 更正了出处），但本库一个都没坐实（RAY-298 的自校验
    没过，另四档连测都没测过）。这里钉住的只是「放行哪七个编码」，理由是防误写，
    见 `Bandwidth` 文档。

    两个集合的**依据不同**，别因为写在同一个函数里就以为它们同源：速率那个集合是
    实测结论（RAY-171），带宽这个集合是协议文档的边界（RAY-304）。
    """
    assert {int(rate) for rate in ReturnRate} == {0x06, 0x07, 0x08, 0x09, 0x0B}
    assert 0x0A not in {int(rate) for rate in ReturnRate}
    assert {int(bandwidth) for bandwidth in Bandwidth} == {
        0x00,
        0x01,
        0x02,
        0x03,
