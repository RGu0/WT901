...
def test_only_verified_rate_and_bandwidth_values_exist() -> None:
    ...
    **带宽七档的情形不同，别把这条测试读成「带宽也逐档实测过」**：七个标称值全部
    来自维特通用编码表，本库一个都没坐实（RAY-298 的自校验没过，另四档连测都没测
    过）。这里钉住的只是「放行哪七个编码」，理由是防误写，见 `Bandwidth` 文档。

    两个集合的**依据不同**，别因为写在同一个函数里就以为它们同源：速率那个集合是
    实测结论（RAY-171），带宽这个集合是上游表的边界（RAY-304）。
    """
