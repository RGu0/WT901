...
def magnetic_field_to_ut(mag_type: int, raw: int) -> float | None:
    ...
    **按本项目方针「官方资料互相矛盾处先立 Issue、以实测定论」，这条已单独立项取证
    （RAY-312），不在实现里选边。** 判据已在那条 Issue 里预注册，取证脚本是
    ``tools/probe_mag_scale.py``。

    ⚠ **判定它没有「读一次模长看合不合理」那么简单。** 那个判据只分得开三份里的
    一份：能不能用地磁量级分开 SDK 与协议文档，**完全取决于本机的** ``0x72``——
    type 3/6/7 差 5–15 倍（能分开），type 2/4 差 1.5–1.7 倍（勉强），而
    **type 5 只差 2%，本方法注定分不开**。加上硬磁偏置与真实场强在静置单点读数里
    不可分，判据改成了「转动拟合球面取半径」。细节见 RAY-312 与
    ``docs/protocol.md`` §5.7。

    在定论之前，拿本函数的返回值做**绝对场强**判断的调用方要知道**它可能整体差
    1.5 倍或 18 倍**；只用它做**相对变化或方向**的调用方不受影响。
    """
    if mag_type == 2:
        return raw * 0.15
    if mag_type == 3:
