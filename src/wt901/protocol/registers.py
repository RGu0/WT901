...
class Bandwidth(IntEnum):
    """写入 :attr:`Register.BANDWIDTH` 的传感器抗混叠滤波器带宽。

    **三档的标称值全部来自维特通用编码表，本库一档都没实测过。** 这与
    ...
    **同样不算数**。失败的成因有两种，那一轮分不开：要么方法无效，要么 ``0x04``
    ...
    要推翻或坐实这三个数字，需要一个动态范围足够的实验——那次实测里噪声底只有约
    20:1 的可用范围，−3 dB 拐点本就测不准。

    **不登记未列出的编码**（通用表还有 ``0x01``=188、``0x02``=98、``0x05``=10、
    ``0x06``=5 Hz）：拒绝未登记编码防的是**误**写，这一点与证据强度无关，仍然成立。
    """
