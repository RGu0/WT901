...
**结论（2026-08-27，一台 WT901BLE67）：8 个。** 两条判据各 3 轮全部成立，
``REGISTERS_PER_RESPONSE`` 已据此从 4 改正为 8。以下是取证当时的原文。

本库的 ``REGISTERS_PER_RESPONSE`` 曾是 4，但三处证据都指向 8：18 字节负载减去 2 字节
...
器：取证时设备层只解前 4 个寄存器，而这里要看的正是整个 18 字节负载。绕开被测代码是
本脚本能当证据用的前提——用库自己的解读去验证库的解读，什么也证明不了。
"""
