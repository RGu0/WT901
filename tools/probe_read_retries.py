...
## 取证结果（2026-09-02）—— 上面的判据一个字未改

**测不出。** 2 台设备 × 各 3 次连接（正好是预注册的样本量），六次运行作废闸零触发：

* **1080 次读零失败**——零超时、零回帧内容异常、零非超时错误。
* early − late 失败率差 **6/6 全是 0 个百分点**（门槛 15）。
* 耗时中位比 **6/6 全在 1.00–1.01**（门槛 1.5）。
* 方向一致 **6/6**（门槛 ≥ 4/6）。

**这是「测不出」，不是「不存在」。** 官方 SDK 那 5 秒与 ``0x72`` 双发既未被证实也
未被否证；``DEFAULT_READ_RETRIES`` 维持 2 不动，因为实测没给出改动它的依据。

附带两条：early 相 360 次零失败给出失败率 95% 置信上界约 **0.83%**（约束效应大小，
不等于否证）；耗时被 BLE 连接间隔量化成 ~60 / ~90 ms 两档，**耗时判据的 1.5 倍门槛
恰好等于「多跨一个连接间隔」**，比这更细的差别本方法看不见。

结论与局限记在 ``docs/protocol.md`` §10，证据在
``ray-313/read-retry-verdict/acceptance/``。
"""
