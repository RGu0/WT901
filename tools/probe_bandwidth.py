...
:data:`VERDICT_BAND`。主判据因此**只对标称 ≤42 Hz 的档位有判读能力**，见
:data:`VERDICT_SCOPE_MAX_HZ`。
...
2. **主判据**：``R̂_0x03`` 落在 :data:`VERDICT_BAND` → 判定为 42 Hz，登记 ``HZ_42``。
   **适用范围限于标称 ≤**:data:`VERDICT_SCOPE_MAX_HZ`：区间把 10 / 20 Hz 排除在外，
   但红噪声重标定后 98 Hz 不再全在区间外，所以本判据对 ≥98 Hz 的档位不给判读
   （RAY-330）。
3. 落在区间外或无拐点 → 本固件不按维特通用表映射这个编码，**不登记**（同 ``0x0A``）。
4. 第 1 条不过 → 方法无效，退回 Issue 描述里的方向 3。

**附加门槛四道**，任一不过就不做「凑合判读」。前三道不过是「重采」，第四道不过是
「本方法测不出来」——那是一种完整交付，不是重采能救的。全部登记在
:data:`GATE_LEDGER`：
...
## 判据的两处修订（红噪声重标定暴露，均已处置）

1. **通带平台绝对水平那道门槛已退役**（RAY-315）。它的标定用的是白噪声，真机是红
   噪声，两轮真机数据被它全部误判为「归一化基准失效、请重采」。换成红噪声之后它既
   分不开干净与污染，要防的失败模式也不再存在。详见 :data:`PLATEAU_LEVEL_CLEAN`。
2. **主判据的适用范围已显式限定在标称 ≤42 Hz**（RAY-330）。红噪声下真值 98 Hz 有
   一组读作 78.9 Hz，落进 :data:`VERDICT_BAND`。**区间值未动**，改为限定范围——成因
   是估计量在观测频段边缘的方差，不是判据被削弱（实测：造成泄漏的那次下穿是「持续」
   判据满窗校验过的）。详见 :data:`VERDICT_SCOPE_MAX_HZ`。

   收窄区间上沿在技术上可行，但需要比现有 9 个样本多得多的标定；而本方法已因动态
   范围不足退役，主判据轮不到生效。记在 **RAY-336**，触发条件是本方法被重新启用。

**仍未修的一条局限**：``_cutoff`` 的「持续」判据窗口在下穿点接近奈奎斯特时会被截断，
而它自己不报告这件事——96.9 Hz 那次只校验了名义窗口的 19% 就宣布「持续」，与满窗
校验的结论在输出里长得一模一样。同样记在 RAY-336。
...
**它只对标称 ≤42 Hz 的档位有判读能力**，见 :data:`VERDICT_SCOPE_MAX_HZ`。区间值
本身自 RAY-304 起未动过。
"""

VERDICT_SCOPE_MAX_HZ = 42.0
"""判据 2 的**适用范围上限**（标称赫兹）。超过它的档位，本方法给不出判读。

RAY-330 确认。为什么要有这条限定：

RAY-315 把合成标定的源噪声换成红噪声之后，真值 98 Hz 开始测得出拐点了（白噪声下
它永远测不出——能量全在低频，奈奎斯特附近没东西可看）。9 组里 3 组测出，读作
78.9–96.9 Hz，其中 **78.9 Hz 落进了** :data:`VERDICT_BAND`——主判据分不开 42 与 98。

**成因不是判据被削弱，是估计量在观测频段边缘的方差。** RAY-330 复现同一批种子并
逐次记录 :func:`_cutoff` 的「持续」判据实际考察了多少 bin：

======  ==========================  ================
检出    「持续」判据实际看到        落进判据区间？
======  ==========================  ================
78.9    9.69 Hz / 26 bin（满窗）    **是**
96.9    1.94 Hz / 6 bin（被截断）   否
96.6    2.33 Hz / 7 bin（被截断）   否
======  ==========================  ================

造成泄漏的那一次是**满窗校验**过的真实下穿，一个 bin 都没少。以 22 段 Welch 平均、
在观测频段边缘 SNR 已经塌下去的地方，估计量的方差足以造成约 20% 的低估。

.. warning::

   **另一条局限：「持续」判据的窗口在下穿点接近奈奎斯特时会被截断，而它自己不报告
   这件事。** 上表里 96.9 Hz 那次只校验了名义窗口的 19% 就宣布「持续」，与 78.9 Hz
   那次校验了满窗得到的结论，**在输出里长得一模一样**。两者的置信度差着数倍，调用方
   看不出来。

   本脚本不修它——修它属于扩样本重定上沿那条路（RAY-336），而那条路的触发条件是
   本方法被重新启用。

**为什么是限定范围而不是收窄区间。** 收窄上沿在技术上可行（78.9 是真实下穿，不是
伪像），但要断言「42 Hz 的估计在更多阶数与更多随机实现下都不会越过新上沿」，靠
RAY-315 那 9 个样本（1–3 阶 × 3 组）撑不住。而 :data:`MIN_DYNAMIC_RANGE_DB` 在干净
合成数据上就不过（14.5 dB，要求 ≥40 dB），本方法已经退役，**主判据根本轮不到生效**
——为一条已退役的路径做扩样本标定，性价比不对。RAY-336 记着它，触发条件是本方法
被重新启用（例如换成主动激励）。
"""
...
def _self_test(seconds: float) -> int:
    ...
    print(f"主判据适用范围：标称 ≤{VERDICT_SCOPE_MAX_HZ:.0f} Hz 的档位"
          f"（RAY-330；超出者只报告，不判定）\n")
    ...
    if not forty_two or not all(low <= v <= high for v in forty_two):
        ...
        ok = False
    # 主判据只对标称 <= VERDICT_SCOPE_MAX_HZ 的档位有判读能力（RAY-330）。
    # 范围内的邻档仍须被排除；超出范围的如实报告，但不参与判定。
    neighbours = (10.0, 98.0, 188.0)
    in_scope = [t for t in neighbours if t <= VERDICT_SCOPE_MAX_HZ]
    out_of_scope = [t for t in neighbours if t > VERDICT_SCOPE_MAX_HZ]

    leaked = [
        truth for truth in in_scope if any(low <= v <= high for v in observed[truth])
    ]
    for truth in leaked:
    ...
    for truth in out_of_scope:
        inside = [v for v in observed[truth] if low <= v <= high]
        detected = len(observed[truth])
        if inside:
            print(
                f"\n⊘ 真值 {truth:.0f} Hz 有 {len(inside)}/{detected} 组落进主判据区间"
                f"（最低 {min(inside):.1f} Hz）——**超出适用范围，不判失败**。"
            )
        elif detected:
            print(
                f"\n⊘ 真值 {truth:.0f} Hz 测出 {detected} 组拐点，均在区间外"
                "——超出适用范围，不参与判定。"
            )
        else:
            print(
                f"\n⊘ 真值 {truth:.0f} Hz 全部无拐点——超出适用范围，不参与判定。"
            )
    if out_of_scope:
        print(
            f"\n   适用范围限在标称 ≤{VERDICT_SCOPE_MAX_HZ:.0f} Hz，理由是估计量在观测频段\n"
            "   边缘的方差：真值 98 Hz 的一组读作 78.9 Hz（低估约 20%），而那次下穿是\n"
            "   「持续」判据满窗校验过的，不是伪像。收窄区间上沿在技术上可行，但要断言\n"
            "   42 Hz 的估计在更多阶数与随机实现下都不越过新上沿，9 个样本撑不住——\n"
            "   记在 RAY-336，触发条件是本方法被重新启用。详见 VERDICT_SCOPE_MAX_HZ。"
        )
    print(
