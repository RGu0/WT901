...
### 带宽档位是另一回事：三个标称值都没实测过

`Bandwidth` 开放 `0x00`（256 Hz）、`0x03`（42 Hz）、`0x04`（20 Hz），但**这三个数字
全部来自维特通用编码表，本库一个都没坐实**——别把它与上面那张逐档实测的速率表看成
同一种东西。
...
那次确立的是两件不依赖标定的事：设备接受这三个编码并照常出数；三档截止频率单调
有序（`0x00` 最宽、`0x04` 最窄、`0x03` 居中）。**拒绝未登记的编码照旧**——那道防线
防的是误写，与证据强度无关。
